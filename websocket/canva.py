...
class basketball():
    ...
    hoop = ring(pos=vector(0, hoopheight+heightadjust, backboard.pos.z+d_fromboard), axis=vector(0, 0.01, 0), radius = 0.4, thickness=0.03, color=color.red)
    ...
    def shoot(self, ax = 0, ay = 0, az = 0):
        ...
        if self.level == 0 : self.basketball.velocity = vector(0, 8, -3)
        else : self.basketball.velocity = vector(0, 9, -4)
