...
class basketball:
    ...
    # 生成籃球場地板
    court = box(
        pos=vector(0, heightadjust, 0), size=vector(7.5, 0.1, 7), color=color.white
    )
    ...
    hoop = ring(
        pos=vector(0, 3.05 + heightadjust, backboard.pos.z + 0.23),
    ...
    # 生成籃球
    basketball = sphere(
        pos=vector(0, 2.1 + heightadjust, 2.9 - 0.12),
        radius=0.12,
        color=color.orange,
        make_trail=False,
    ...
    def shoot(self, ax=0, ay=0, az=0):
        ...
        while True:
            ...
            #     return False
            if (
                self.basketball.pos.x > 3.75
                or self.basketball.pos.x < -3.75
                or self.basketball.pos.z > 3.5
                or self.basketball.pos.z < -3.5 + self.basketball.radius
            ):
                time = 0
                self.basketball.pos = vector(0, 1.9 + self.heightadjust, 2.9 - 0.12)
                return False
            ...
            if (
                (
                    ...
                    <= self.backboard.pos.z + 0.05 + self.basketball.radius
                )
                and (
                    self.basketball.pos.z
                    >= self.backboard.pos.z - 0.05 - self.basketball.radius
